...
WarbandComms.EnqueueRosterUpdate = false
WarbandComms.CurrentRosterType = "solo"
WarbandComms.CurrentRosterLabel = "solo"
...
function WarbandComms.GetOutboundChatRoute()
	if IsWarBandActive and IsWarBandActive() then
		return {
			command = "/wb ",
			shortCommand = "/wb",
			chatType = SystemData.ChatLogFilters.BATTLEGROUP,
			label = "warband",
		}
	end

	if GameData and GameData.Player and GameData.Player.isInScenario then
		return {
			command = "/sc ",
			shortCommand = "/sc",
			chatType = SystemData.ChatLogFilters.SCENARIO,
			label = "scenario",
		}
	end

	if PartyUtils and PartyUtils.IsPartyActive and PartyUtils.IsPartyActive() then
		return {
			command = "/g ",
			shortCommand = "/g",
			chatType = SystemData.ChatLogFilters.GROUP,
			label = "group",
		}
	end

	if WarbandComms.selfTest then
		return {
			command = "/say ",
			shortCommand = "/say",
			chatType = SystemData.ChatLogFilters.SAY,
			label = "self-test",
		}
	end
...
function WarbandComms.GetActiveRosterRoute()
	if IsWarBandActive and IsWarBandActive() then
		return {
			type = "warband",
			label = "warband",
	...
	if GameData
		and GameData.Player
		and (GameData.Player.isInScenario or GameData.Player.isInSiege)
		and GameData.GetScenarioPlayerGroups then
		return {
			type = "scenario",
			label = "scenario",
		}
...
function WarbandComms.GetAcceptedInboundChannelLabels()
	return {
		"warband (/wb)",
		"scenario (/sc)",
		"group (/g)",
...
local function GetRosterSortBonus(careerLine)
	local sortAdder = {
	...
	return sortAdder[careerLine] or 0
end
...
local function GetMemberCareerLine(member)
	...
	if member.careerLine then
		return member.careerLine
	end

	if member.career and member.career.line then
		return member.career.line
	end

	if member.careerId and WarbandComms.CareerIdToLine then
		return WarbandComms.CareerIdToLine[member.careerId]
	end

	if member.career and WarbandComms.CareerIdToLine then
		return WarbandComms.CareerIdToLine[member.career]
	end

	return nil
end

local function GetLocalPlayerCareerLine()
	if GameData and GameData.Player and GameData.Player.career then
		if GameData.Player.career.line then
			return GameData.Player.career.line
		end

		if GameData.Player.career.id and WarbandComms.CareerIdToLine then
			return WarbandComms.CareerIdToLine[GameData.Player.career.id]
		end
	end
...
local function NormalizeMemberName(name)
	...
	local normalized = tostring(WarbandComms.FixString(name))
...
local function GetLocalPlayerName()
	if not GameData or not GameData.Player or not GameData.Player.name then
		return nil
	end

	return NormalizeMemberName(GameData.Player.name)
end

local function AddOrderedRosterMember(orderedMembers, member)
	if member then
		orderedMembers[#orderedMembers + 1] = member
	end
end

local function EnsureLocalRosterMember(orderedMembers)
	local localPlayerName = GetLocalPlayerName()
	if not localPlayerName or localPlayerName == "" then
		return
	end

	for _, member in ipairs(orderedMembers) do
		local memberName = NormalizeMemberName(member and member.name)
		if memberName == localPlayerName then
			return
		end
	end

	orderedMembers[#orderedMembers + 1] = {
		name = localPlayerName,
		careerLine = GetLocalPlayerCareerLine(),
	}
end

local function BuildRosterMapFromOrderedMembers(orderedMembers)
	local rosterMap = {}
	local seenNames = {}

	for _, member in ipairs(orderedMembers or {}) do
		local name = NormalizeMemberName(member and member.name)
		if name and not seenNames[name] then
			local index = #rosterMap + 1
			...
			rosterMap[index] = {
				...
				sortOrder = (1000 - index) + GetRosterSortBonus(careerLine),
			}
			seenNames[name] = true
		end
	end
	...
	if #rosterMap == 0 then
		local localPlayerName = GetLocalPlayerName()
		if localPlayerName and localPlayerName ~= "" then
			rosterMap[1] = {
				name = localPlayerName,
				index = 1,
				careerLine = GetLocalPlayerCareerLine(),
...
local function BuildOrderedWarbandMembers(warband)
	local orderedMembers = {}

	for _, party in ipairs(warband or {}) do
		for _, member in ipairs((party and party.players) or {}) do
			AddOrderedRosterMember(orderedMembers, member)
		end
	end
...
local function BuildOrderedGroupMembers(groupMembers)
	local orderedMembers = {}

	for _, member in ipairs(groupMembers or {}) do
		AddOrderedRosterMember(orderedMembers, member)
	end
...
local function BuildOrderedScenarioMembers(scenarioPlayers)
	local orderedMembers = {}
	local groupedMembers = {}
	local ungroupedMembers = {}

	for _, player in ipairs(scenarioPlayers or {}) do
		local groupIndex = tonumber(player and player.sgroupindex) or 0
		local slotIndex = tonumber(player and player.sgroupslotnum) or 0
		if groupIndex > 0 and slotIndex > 0 then
			groupedMembers[((groupIndex - 1) * 6) + slotIndex] = player
		else
			ungroupedMembers[#ungroupedMembers + 1] = player
		end
	end

	for slotIndex = 1, 36 do
		AddOrderedRosterMember(orderedMembers, groupedMembers[slotIndex])
	end

	for _, player in ipairs(ungroupedMembers) do
		AddOrderedRosterMember(orderedMembers, player)
	end

	return orderedMembers
end

local function ApplyRosterMap(rosterType, rosterMap)
	WarbandComms.WarbandMap = rosterMap
	WarbandComms.CurrentRosterType = rosterType or "solo"
	WarbandComms.CurrentRosterLabel = (rosterType == "group" and "group")
		or (rosterType == "scenario" and "scenario")
		or (rosterType == "warband" and "warband")
		or "solo"

	if not WarbandComms.InWarband and rosterType == "warband" then
...
function WarbandComms.PrintSelfCheck()
	...
	local outboundRoute = WarbandComms.GetOutboundChatRoute()
	local rosterRoute = WarbandComms.GetActiveRosterRoute()
	local accepted = table.concat(WarbandComms.GetAcceptedCommsKeys(), ", ")
	local acceptedChannels = table.concat(WarbandComms.GetAcceptedInboundChannelLabels(), ", ")
	...
	if outboundRoute then
		EA_ChatWindow.Print(towstring("[WarbandComms] Outbound chat route: " .. outboundRoute.label .. " (" .. outboundRoute.shortCommand .. ")"))
	else
		EA_ChatWindow.Print(towstring("[WarbandComms] Outbound chat route: inactive"))
	end
	EA_ChatWindow.Print(towstring("[WarbandComms] Route precedence: warband > scenario > group > self-test"))
	...
	EA_ChatWindow.Print(towstring("[WarbandComms] Accepted inbound chat routes: " .. acceptedChannels))
	EA_ChatWindow.Print(towstring("[WarbandComms] Active roster source: " .. rosterRoute.label .. " (" .. tostring(#(WarbandComms.WarbandMap or {})) .. " mapped members)"))
...
function WarbandComms.OnInitialize()
	local function RegisterIfAvailable(eventId, handlerName)
		if eventId then
	...
	RegisterIfAvailable(SystemData.Events.BATTLEGROUP_UPDATED, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.GROUP_UPDATED, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.GROUP_PLAYER_ADDED, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.GROUP_LEAVE, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.SCENARIO_BEGIN, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.CITY_SCENARIO_BEGIN, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.SCENARIO_END, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.CITY_SCENARIO_END, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.SCENARIO_GROUP_JOIN, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.SCENARIO_GROUP_LEAVE, "WarbandComms.OnRosterUpdated")
	RegisterIfAvailable(SystemData.Events.SCENARIO_PLAYERS_LIST_GROUPS_UPDATED, "WarbandComms.OnRosterUpdated")
	...
	if WarbandComms.StartTest then
		EA_ChatWindow.Print(towstring("[WarbandComms] TEST BUILD ACTIVE: /wbc testboxes, /wbc testgroup, /wbc testscenario, /wbc testcenter, /wbc selftest"))
	end
end

function WarbandComms.OnRosterUpdated()
	WarbandComms.EnqueueRosterUpdate = true

end

function WarbandComms.MapBattleGroupMembers(...)
	return WarbandComms.MapWarbandMembers(...)
end

function WarbandComms.MapRosterMembers(rosterType, rosterData, options)
	local orderedMembers = {}
	local includeLocalPlayer = not options or options.includeLocalPlayer ~= false

	if rosterType == "warband" then
		orderedMembers = BuildOrderedWarbandMembers(rosterData)
	elseif rosterType == "scenario" then
		orderedMembers = BuildOrderedScenarioMembers(rosterData)
	elseif rosterType == "group" then
		orderedMembers = BuildOrderedGroupMembers(rosterData)
	end

	if includeLocalPlayer then
		EnsureLocalRosterMember(orderedMembers)
	end

	ApplyRosterMap(rosterType, BuildRosterMapFromOrderedMembers(orderedMembers))
end

function WarbandComms.MapWarbandMembers(warband, options)
	if not warband and PartyUtils and PartyUtils.GetWarbandData then
		warband = PartyUtils.GetWarbandData()
	end

	return WarbandComms.MapRosterMembers("warband", warband or {}, options)
end

function WarbandComms.MapGroupMembers(groupMembers, options)
	if not groupMembers and PartyUtils and PartyUtils.GetPartyData then
		groupMembers = PartyUtils.GetPartyData()
	end

	return WarbandComms.MapRosterMembers("group", groupMembers or {}, options)
end

function WarbandComms.MapScenarioMembers(scenarioPlayers, options)
	if not scenarioPlayers and GameData and GameData.GetScenarioPlayerGroups then
		scenarioPlayers = GameData.GetScenarioPlayerGroups()
	end

	return WarbandComms.MapRosterMembers("scenario", scenarioPlayers or {}, options)
end

function WarbandComms.MapCurrentRosterMembers()
	local rosterRoute = WarbandComms.GetActiveRosterRoute()

	if rosterRoute.type == "warband" then
		return WarbandComms.MapWarbandMembers()
	end

	if rosterRoute.type == "scenario" then
		return WarbandComms.MapScenarioMembers()
	end

	if rosterRoute.type == "group" then
		return WarbandComms.MapGroupMembers()
	end

	return WarbandComms.MapRosterMembers("solo", {}, { includeLocalPlayer = true })
end
...
function WarbandComms.OnUpdate(elapsed)
    ...
    if WarbandComms.elapsed - (WarbandComms.lastUIUpdate or 0) >= throttle1 then
		...
		if WarbandComms.EnqueueRosterUpdate then
			WarbandComms.MapCurrentRosterMembers()
			WarbandComms.EnqueueRosterUpdate = false
		end
...
function WarbandComms.OnCast(actionId, isChannel, desiredCastTime, averageLatency)
	...
	local outboundRoute = WarbandComms.GetOutboundChatRoute()
	if not outboundRoute then return end
	...
	if enabled then
		...
		local message = outboundRoute.command .. WarbandComms.GetOutboundCommsKey() .. ":" .. tracker .. ":" .. tostring(duration)
...
function WarbandComms.TextArrived()
    ...
    if WarbandComms.ChatChannels[chatType] then
		...
		local sender = NormalizeMemberName(chatData.name)
