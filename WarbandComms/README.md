...
- Auto-routes outbound comms through warband (`/wb`), scenario (`/sc`), or group (`/g`) chat based on the active battle-group context
...
- `selfcheck` - prints protocol, routing, and runtime diagnostics in chat
- `testboxes` - runs local tracker-box test data flow in test builds (`test` also works as a compatibility alias)
- `testgroup` - runs the tracker-box harness against a synthetic 6-player group roster in test builds
- `testscenario` - runs the tracker-box harness against a synthetic scenario roster in test builds
...
- `/wbc testgroup` in a test build
- `/wbc testscenario` in a test build
- `/wbc testcenter` in a test build

Test build notes:

- `testboxes` populates all five tracker boxes, including a dedicated Immaculate Defense box.
- `testgroup` and `testscenario` provide repeatable non-warband roster checks for party/scenario UI rendering.
- Test harness names are generated per run so rows look more like live data while still covering the targeted roster.

## Current Release

- Addon version: `3.6.0`
- Addon comms now send and accept `[WBC]` messages only.
...
- Use `/wbc testboxes`, `/wbc testgroup`, and `/wbc testscenario` in test builds to verify warband, party, and scenario roster mapping through the live UI path.
